...
- 🧩 **Skill 蒸馏库（v2）** — 原点聚合→蒸馏 3-6 个高质量 Skill，扩展仅做证据融合不堆数量
...
Skill 使用说明（对话）：
- 自动触发：系统会按 `intent + semantic + boundary` 匹配 top-2 distilled skills
- 手动触发：在消息前使用 `/skill <技能名>` 强制指定 Skill
